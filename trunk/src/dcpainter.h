 ...
 * @brief   Declaration of DCPainter class responsible for drawing song(s) to various Device Contexts
 */
...
struct TSetLine
{
    ...
    virtual ~TSetLine()
    {
        while (!m_textItems.empty())
        {
            delete m_textItems.back();
            m_textItems.pop_back();
        }
        while (!m_chordItems.empty())
        {
            delete m_chordItems.back();
            m_chordItems.pop_back();
        }
    }
};
...
struct TSetBlock
{
    ...
    void setMaxWidth(wxCoord width)
    {
        mMaxWidth = width;
    };
    wxCoord getMaxWidth()
...
struct TSetBlockSingleLine : public TSetBlock
{
    ...
    virtual void setTxt(wxString text)
    {
        mLine.setText(text);
    };
    virtual void draw();
